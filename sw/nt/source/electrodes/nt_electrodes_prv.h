...
#define ROBUST_AVERAGE_N ( 3 )
#define INIT_BUFFER_N ( 7 )
...
struct nt_electrode_data
{
    ...
    int32_t delta;                      /*!< Delta signal */
    int32_t delta_red;                  /*!< Xtalk reduced delta signal */
    uint8_t delta_red_on;               /*!< Flag to use reduced delta signal */
    uint16_t nstep;                     /*! NSTEP value */
    uint16_t baseline;                  /*!< Baseline. */
    uint16_t count_init;
    uint16_t init_buffer[INIT_BUFFER_N];
    uint16_t init_buffer_count;
...
{
    NT_ELECTRODE_ENABLED =
        1 << NT_FLAGS_SYSTEM_SHIFT(0), /**< Electrode is enabled or disabled. */
    NT_ELECTRODE_LOCK_BASELINE_REQ_FLAG =
        1 << NT_FLAGS_SYSTEM_SHIFT(1), /**< Electrode's baseline should be locked (can't be updated). */
    NT_ELECTRODE_LOCK_BASELINE_FLAG =
        1 << NT_FLAGS_SYSTEM_SHIFT(2), /**< Electrode's baseline is locked (cannot be updated). */
    NT_ELECTRODE_DIGITAL_RESULT_ONLY_FLAG =
        1 << NT_FLAGS_SYSTEM_SHIFT(3), /**< Electrode's event does not have analog information (cannot be used for analog controls). */
    NT_ELECTRODE_AFTER_INIT_TOUCH_FLAG =
        1 << NT_FLAGS_SYSTEM_SHIFT(4), /**< Electrode is touched after the enable/init process. */
    NT_ELECTRODE_EN_GROUND_ADJACENT_FLAG =
        1 << NT_FLAGS_SYSTEM_SHIFT(5), /**< Electrode's channel is enabled to be grounded when not measured. */
    NT_ELECTRODE_EN_SHIELD_ADJACENT_FLAG =
        1 << NT_FLAGS_SYSTEM_SHIFT(6), /**< Electrode's channel is enabled to be shieleded when not measured. */
};
...
/**
 * \brief Scale delta signal.
 * \param electrode A pointer to the electrode data.
 * \param signal
 * \return signal
 *
 * Normalize the delta signal to working values. Values from
 * nt_electrode delta_divider or delta_multiplier normalize the delta signal.
 */
uint16_t _nt_electrode_delta_normalization_process(const struct nt_electrode_data *electrode, int32_t signal)
;
/**
 ...
 * \return none
 */
void _nt_electrode_set_delta(struct nt_electrode_data *electrode, int32_t signal);

/**
 * \brief Set the xtalk reduced delta signal for the electrode.
 * \param electrode A pointer to the electrode.
 * \param signal
 * \return none
 */
void _nt_electrode_set_delta_red(struct nt_electrode_data *electrode, int32_t signal);
...
/**
 * \brief Get the delta signal
 * \param electrode Pointer to the electrode data.
 * \return Delta signal calculated from the last raw value measured.
 *
 * The signal value is calculated from the raw electrode capacitance or other
 * physical signal by applying the filtering and normalization algorithms
 * and subtracting the baseline
 */
uint32_t nt_electrode_get_delta(const struct nt_electrode *electrode);

/**
 * \brief Get the xtalk reduced delta signal
 * \param electrode Pointer to the electrode data.
 * \return Delta signal calculated from the last raw value measured.
 *
 * The signal value is calculated from the raw electrode capacitance or other
 ...
 * and subtracting the baseline
 */
uint32_t nt_electrode_get_delta_red(const struct nt_electrode *electrode);
...
/**
 * \brief Return xcorr reduced delta signal.
 * \param electrode Pointer to the electrode data.
 * \return Immediate delta value between the processed signal and its baseline
 *     (idle) value.
 */
int32_t _nt_electrode_get_delta_red(const struct nt_electrode_data *electrode);

/**
 * \brief Return the signal baseline.
 * \param electrode Pointer to the electrode data.
 * \return Immediate baseline (idle) value
 */
int32_t _nt_electrode_get_baseline(const struct nt_electrode_data *electrode);
