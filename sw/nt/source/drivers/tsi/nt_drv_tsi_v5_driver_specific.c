...
tsi_status_t NT_TSI_DRV_Measure(uint32_t instance)
{
    ...
    uint8_t electrode_index  = module->electrodes_cnt;
    bool random_order_enable = module->rom->elec_random_order;
    ...
    /* Randomize the order of electrodes */
    if ((bool)random_order_enable)    
    {
        uint8_t *order = module->electrodes_rand_order_arr;
        uint16_t random_val = TSI_GetCounter(base);
        uint8_t i, j, tmp;
        
        for(i = 0; i < module->electrodes_cnt; ++i)
        {
            /* Pick index for array */
            j = random_val % (i + 1);
            
            /* Swap two electrodes in array */
            tmp      = order[i];
            order[i] = order[j];
            order[j] = tmp;
        }
    ...
    while ((bool)electrode_index--)
    {
        /* Select first electrode to be measured */
        if(random_order_enable)
        {
            electrode_last = module->electrodes_rand_order_arr[electrode_index];
            __DSB();
            __ISB();
        }
        else
        {
            electrode_last = electrode_index;
        }

        /* Find the last enabled electrode */
        if ((bool)_nt_electrode_get_flag(module->electrodes[electrode_last], (uint32_t)NT_ELECTRODE_ENABLED))
        {
            bool gnd_adj_enabled = (bool)_nt_electrode_get_flag(module->electrodes[electrode_last], (uint32_t)NT_ELECTRODE_EN_GROUND_ADJACENT_FLAG);
            ...
            {   bool shield_adj_enabled = (bool)_nt_electrode_get_flag(module->electrodes[electrode_last], (uint32_t)NT_ELECTRODE_EN_SHIELD_ADJACENT_FLAG);
                ...
                /* Adjacent channel shielding */
#if defined(FSL_FEATURE_TSI_HAS_SHIELD_REGISTER) && FSL_FEATURE_TSI_HAS_SHIELD_REGISTER                
                if(shield_adj_enabled)
                {       
                  /* Enable shilding for adjacent electrode to be measured (all electrodes assigned to control are set as shiled) */
                  TSI_ShieldChannelConfig(base, module->electrodes[electrode_last]->rom->shield_mask);
                }      
#endif
            }
...
#if (defined(FSL_FEATURE_TSI_HAS_M_TX_USED) && FSL_FEATURE_TSI_HAS_M_TX_USED)
                /* Enable single Mutual Tx channel and disable rest of them (improved xtalk) */       
                if(gnd_adj_enabled)
                {
                    base->MUL0 &= ~TSI_MUL0_M_TX_USED_MASK;                         /* Disable all */
                    TSI_SetUsedTxChannel(base, (tsi_mutual_tx_channel_t)mutual_tx); /* Enable selected */
                }
#endif    
            }
            ...
            if(gnd_adj_enabled)
            {       
              /* Switch 1st electrode or electrodes (mutual) to be measured to TSI (all electrodes are in GPIO mode from init) */
              (void)_nt_module_safety_switch_electrode_tsi(module->electrodes[electrode_last]->rom);
            }
            ...
            if (tsiState->opSatus != tsi_OpStatusSuspend)
            {
                ...
                /* store TSI channel triggered for sensing to be read in handler*/
                if(module->rom->elec_random_order)
                {
                    module->electrode_last = electrode_index;
                }
                else
                {
                    module->electrode_last = electrode_last;
                }
            }
            return kStatus_TSI_Success;
        }
...
void TSI_DRV_IRQHandler(uint32_t instance)
{
    ...
    uint8_t electrode_index  = electrode_last;  
    bool random_order_enable = module->rom->elec_random_order;
    
    /* Select lastly measured random electrode */
    if(random_order_enable)
    {
        electrode_last = module->electrodes_rand_order_arr[electrode_index];
    }
    ...
    if ((bool)(tsiState->status != kStatus_TSI_Overflow))
    {
      ...
      {                                          /* Save TSICNT value */
        ...
        if (!(bool)(TSI_GetSensingMode(base))) /* self-cap electrode was measured */
        {
          ...
          nstep1 = ( uint32_t )TSI_GetCounter( base );
        ...
        else /* mutual-cap electrode was measured */
        {
          ...
          module->electrodes[electrode_last]->nstep = nstep1; // Needed for Python tool
        }
        
        /* Adjacent channel grounding*/
        if((bool)_nt_electrode_get_flag(module->electrodes[electrode_last], (uint32_t)NT_ELECTRODE_EN_GROUND_ADJACENT_FLAG))
        {    
          /* Switch previously measured electrode to GPIO output low level 
          (while all other electrodes are in GPIO mode from init) */
          (void)(int32_t)_nt_module_safety_switch_electrode_gpio_output_set_low(module->electrodes[electrode_last]->rom);
        }

        if ((bool)(electrode_index == 0U)) /* The last channel was read */
        {
          module->electrode_last              = module->electrodes_cnt; /* reset electrode number */
    ...
    if ((bool)(tsiState->opSatus != tsi_OpStatusSuspend))
    {
        while ((bool)electrode_index--)
        { 
            /* Select next electrode to be measured */
            if (random_order_enable)
            {
                electrode_last = module->electrodes_rand_order_arr[electrode_index];
            }
            ...
            { 
                ...
                if(gnd_adj_enabled)
                {    
                  /* Switch selected electrode or electrodes(mutual) to be measered to TSI 
                  (while all other electrodes are in GPIO mode from init) */
                  (void)(int32_t)_nt_module_safety_switch_electrode_tsi(module->electrodes[electrode_last]->rom);                  
                }
    ...
    while ((bool)(elNumber--))
    {
        do
        {
            ...
            else if (counter > recalib_config->SelfRangeMax) /* untouched counter value is too high */
            {
                ...
                else if (configSelfCap->commonConfig.cutoff > kTSI_SincCutoffDiv_1 &&
                         cycleCounter > 32U + 8U + 2U + 4U &&
                         cycleCounter <= 32U + 8U + 2U + 4U + 8U) /* Cut-off divider calibration, 8 values */
                {
                    uint8_t temp = (uint8_t)configSelfCap->commonConfig.cutoff;
    ...
    while ((bool)(elNumber--))
    {
        do
        {
            ...
            if (delta < (int32_t)recalib_config->SelfRangeDeltaMin)
            {
                ...
                if (configSelfCap->xdn < kTSI_SensitivityXdnOption_3 &&
                    cycleCounter <= 4U) /* Sensitivity XDN option calibration, 8 values   */
                {
                    ...
                    continue;
                         ...
                         cycleCounter <= 4U + 8U) /* Sensitivity CTRIM option calibration, 8 values */
                {
                ...
                else if (configSelfCap->commonConfig.dvolt > kTSI_DvoltOption_0 && cycleCounter > 4U + 8U &&
                         cycleCounter <= 4U + 8U + 4U) /* DVOLT option value calibration, 4 values */
                {
                    uint8_t temp = (uint8_t)configSelfCap->commonConfig.dvolt;
                    temp -= 1U;
                    configSelfCap->commonConfig.dvolt = (tsi_dvolt_option_t)temp;
                ...
                else if (configSelfCap->commonConfig.cutoff > kTSI_SincCutoffDiv_1 && cycleCounter > 4U + 8U + 4U &&
                         cycleCounter <= 4U + 8U + 4U + 8U) /* Cut-off divider calibration, 8 values */
                {
                    uint8_t temp = (uint8_t)configSelfCap->commonConfig.cutoff;
                    temp -= 1U;
                    configSelfCap->commonConfig.cutoff = (tsi_sinc_cutoff_div_t)temp;
                ...
                else if (cycleCounter > 0U && cycleCounter <= 4U + 8U + 4U + 8U)
                {
                    continue;
                }
                else
...

            {
                ...
                else if (configSelfCap->commonConfig.dvolt < kTSI_DvoltOption_3 && cycleCounter > 4U + 8U &&
                         cycleCounter <= 4U + 8U + 4U) /* DVOLT option value calibration, 4 values */
                {
                    uint8_t temp = (uint8_t)configSelfCap->commonConfig.dvolt;
                    temp += 1U;
                ...
                else if (configSelfCap->commonConfig.cutoff < kTSI_SincCutoffDiv_128 && cycleCounter > 4U + 8U + 4U &&
                         cycleCounter <= 4U + 8U + 4U + 8U) /* Cut-off divider calibration, 8 values */
                {
                    uint8_t temp = (uint8_t)configSelfCap->commonConfig.cutoff;
                    temp += 1U;
    ...
    while ((bool)(elNumber--))
    {
        do
        {
            ...
            if (counter < recalib_config->MutRangeMin) /* untouched counter value is too low */
            {
                ...
                else if (configMutual->commonConfig.cutoff > kTSI_SincCutoffDiv_1 && cycleCounter > 32U &&
    ...
    while ((bool)(elNumber--))
    {
        do
        {
            ...
            if (delta < (int32_t)recalib_config->MutRangeDeltaMin)
            {
                ...
                else if (configMutual->commonConfig.cutoff > kTSI_SincCutoffDiv_1 && cycleCounter > 32U + 32U &&
