...
uint64_t _nt_module_get_electrodes_state(struct nt_module_data *module)
{
    ...
    {
        uint64_t electrode_state = _nt_electrode_is_touched(module->electrodes[elec_counter]);
...
struct nt_module_data *_nt_module_init(const struct nt_module *module)
{
    ...
    for (i = 0; i < _this->electrodes_cnt; i++)
    {
        ...
        /* If pin input is incorrectly configurated */
        if ((bool)(_this->electrodes[i]->rom->pin_input >= TF_TSI_TOTAL_CHANNEL_COUNT))
        {
            return NULL;
        }     
    }      
    
    /* Init array for measurement of module's electrodes in random order */
    if ((bool)_this->rom->elec_random_order)
    {
        _this->electrodes_rand_order_arr = _nt_mem_alloc((uint32_t)sizeof(_this->electrodes_rand_order_arr) * (uint32_t)_this->electrodes_cnt);
    
        if (_this->electrodes_rand_order_arr == NULL)
        {
            return NULL;
        }
        
        /* Set ascending order of used electrodes */
        for(i = 0; i < _this->electrodes_cnt; ++i)
        {
            _this->electrodes_rand_order_arr[i] = i;
        }
    }
