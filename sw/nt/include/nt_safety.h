 ...
 * \return The test result status (success or failure).
 *
 * The function read the gpio and pin information from gpio_input member of the electrode structure
...
/**
 * \brief Function change electrode pin mode from TSI to GPIO and set pin level to low.
 * \return The test result status (success or failure).
 *
 * The function read the gpio and pin information from gpio_input member of the electrode structure
 * and change this pin to GPIO mode and set pin level to low.
  \code
    struct nt_module_data *module = _nt_module_get_data(_nt_system_get_module((uint32_t)&nt_module_tsi_interface,
 instance)); NT_ASSERT(module != NULL);
    struct nt_electrode const *electrode = module->rom->electrodes[0];
    int32_t Result = _nt_module_safety_switch_electrode_gpio_output_set_low(electrode);
  \endcode
 */
int32_t _nt_module_safety_switch_electrode_gpio_output_set_low(const struct nt_electrode *elec);
