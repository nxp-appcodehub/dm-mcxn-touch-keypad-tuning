 ...
 */
status_t ITRC_GetStatus1(ITRC_Type *base)
{
    return base->STATUS1;
}
    ...
    ITRC_Type *base, itrc_out_signals_t out, itrc_input_signals_t in, itrc_lock_t lock, itrc_enable_t enable)
{
    ...
    if (in < OUT_SEL_0_COUNT)
        index = 2u * in;
    else if (OUT_SEL_0_COUNT <= in && in < OUT_SEL_1_COUNT)
        index = 2u * (in - OUT_SEL_0_COUNT);
    else if (OUT_SEL_1_COUNT <= in && in < OUT_SEL_2_COUNT)
        index = 2u * (in - OUT_SEL_1_COUNT);
    else
        return kStatus_InvalidArgument;
    ...
    for (uint8_t i = kITRC_Irq; i < ITRC_OUT_COUNT; i++)
    {
