...
/* Reset CONTROL mask */
#define RESERVED_CTRL_MASK 0x800

#if defined(CDOG_IRQS)
/* Array of IRQs */
static const uint32_t s_CdogIrqs[] = CDOG_IRQS;
#endif /* CDOG_IRQS */
...
status_t CDOG_Init(CDOG_Type *base, cdog_config_t *conf)
{
    ...
    for (int i = 0; i < ARRAY_SIZE(s_CdogIrqs); i++)
    {
        NVIC_EnableIRQ((IRQn_Type)s_CdogIrqs[i]);
    }
#endif /* CDOG_IRQS */
...
void CDOG_Deinit(CDOG_Type *base)
{
    ...
    for (int i = 0; i < ARRAY_SIZE(s_CdogIrqs); i++)
    {
        NVIC_DisableIRQ((IRQn_Type)s_CdogIrqs[i]);
    }
#endif /* CDOG_IRQS */
